<%@ Page Title="" Language="C#" MasterPageFile="~/Inside.master" AutoEventWireup="true" CodeFile="search_labstuff.aspx.cs" Inherits="search_labstuff" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphBreadcrumb" Runat="Server"><a href="/">Home</a> > Laboratory Products</asp:Content>
<asp:Content ID="Content3" ContentPlaceHolderID="cphBody" Runat="Server">
    <h1>Laboratory Products - <%=String.Format("{0:##,###,##0}", ProductCount)  %> items</h1>
            ...
            <table width="958" class="tablesorter" align="center" >
                ...
                <tbody>
                    <asp:ListView ID="lvProducts" runat="server" DataSourceID="dataProducts">
                        ...
                        <ItemTemplate>
                            <tr>
                                <td class="desc"><b><a href='products/product_labstuff.aspx?part=<%# Eval("partnumber") %>'><%# Eval("partnumber") %></a></b></td>
						...
						<table class="mobile-result-table">
							<tr>
								<td colspan="4" class="fullTD">
									<a href='products/product_labstuff.aspx?part=<%# Eval("partnumber") %>'><%# Eval("title") %></a>
								</td>
							</tr>
							<tr>
								<td class="headerColumns" colspan="2" style="width:35%">Part #</td>
								<td class="topBorderTD" colspan="2"><a href='products/product_labstuff.aspx?part=<%# Eval("partnumber") %>'><%# Eval("partnumber") %></a></td>
							</tr>
							<tr>
    ...
    <asp:SqlDataSource ID='dataProducts' runat="server" ConnectionString="<%$ ConnectionStrings:SPEXCertiPrep %>" ProviderName="<%$ ConnectionStrings:SPEXCertiPrep.ProviderName %>"
        SelectCommand="SELECT cp.cpPart AS PartNumber, cp.cpDescrip AS Title, cp.cpUnitCnt AS UnitPack FROM  cp_roi_Prods cp WHERE cp.cpType = 6 AND cp.For_Web = 'Y' ORDER BY cp.cpDescrip" SelectCommandType="Text" onselected="dataProducts_Selected">
    </asp:SqlDataSource>
