...
dim applicationurl as string="http://spexcsp.bnoinc.com/chat/"
sub page_load()
	'/// Do browser check ///'
	...
	'/// Anyone On-line'
	if u="" and d="" then 
		if onlinestatus then img="chat_active.gif" else img="chat_inactive.gif"
	end if
	
	'/// Check For File'
	if img<>"chat_active.gif" and img<>"chat_inactive.gif" then
...
function xlaALScheckstatus_<%=randomid%>(){
	...
	document.getElementById("xlaALSstatusimg_<%=randomid%>").src='<%=applicationurl%>als-ie6.aspx?u=<%=u%>&d=<%=d%>&getstatus=1&nt=' + nt;
